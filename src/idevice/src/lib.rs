...
pub trait IdeviceService: Sized {
    ...
    async fn connect(provider: &dyn IdeviceProvider) -> Result<Self, IdeviceError> {
        ...
        debug!("Before start sevice");
        let (port, ssl) = lockdown.start_service(Self::service_name()).await?;
        debug!("After start service");
        ...
        debug!("Before ssl if");
        if ssl {
            debug!("Inside ssl if, before start_session");
            idevice
                .start_session(&provider.get_pairing_file().await?)
                .await?;
            debug!("Inside ssl if, after start_session");
        }

        debug!("Before set_udid");
        if let Some(udid) = udid_value {
            idevice.set_udid(udid);
        }
        debug!("After set_udid");
    ...
    pub async fn start_session(
        ...
        if CryptoProvider::get_default().is_none() {
            ...
            let crypto_provider: CryptoProvider = {
                debug!("Choosing crypto provider");
            ...
            debug!("Before install crypto provider");
            ...
            debug!("After install crypto provider");
        }
